...
<h1 class="text-3xl font-bold mb-4">記事を修正する</h1>
<form method="POST" action="{{ route('articles.update', $article) }}" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    <div class="mb-4">
        <label for="title" class="block text-sm font-medium text-gray-700 mb-1">タイトル</label>
        <input type="text" name="title" id="title" value="{{ $article->title }}" class="w-full border border-gray-300 px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200" required>
    </div>
    <div class="mb-4">
    ...
    <div id="article-editor" class="border border-gray-300 px-3 py-2 min-h-48 text-base leading-relaxed" data-content="{{ e($article->body) }}"></div>
    </div>
        <div class="mb-4">
            <label for="image" class="inline-block border border-gray-400 px-4 py-1 text-sm cursor-pointer hover:bg-gray-50">
                画像を選ぶ
            </label>
            <input type="file" name="image_path" id="image" class="w-full">
            @if($article->image_path)
            <div class="mb-2">
                <img src="{{ asset('storage/' . $article->image_path) }}" alt="" class="w-32">
...
<script>window.articleContent = {!! json_encode($article->body) !!};</script>
</main> 
